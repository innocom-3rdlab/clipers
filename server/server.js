...
async function main() {
    ...
    router.get('/jobs/:jobId/analysis', authenticateToken, (req, res) => { const job = jobs.get(req.params.jobId); if (!job || job.userId !== req.user.id) return res.status(404).json({ message: 'Job not found.' }); if (!job.analysisData) return res.status(404).json({ message: 'Analysis data not available for this job.' }); res.json(job.analysisData); });
...
const cleanup = () => {
    ...
    server.close(() => {
        ...
        const tempDirs = [DOWNLOAD_DIR, CLIPS_DIR, AUDIO_DIR];
        tempDirs.forEach(dir => {
            if (fs.existsSync(dir)) {
                fs.rm(dir, { recursive: true, force: true }, err => {
                    if (err) console.error(`一時ディレクトリのクリーンアップに失敗: ${dir}`, err);
                    else console.log(`クリーンアップ完了: ${dir}`);
                });
            }
        });
        process.exit(0);
